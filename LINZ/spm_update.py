...
class StationCoordModelUpdater(object):
    ...
    def read_config(self, cfgfile, options=None):
        ...
        self.auto_reject_obs = (
            str(config.get("auto_reject_obs", "False")).lower() == "true"
        )
    ...
    def _calcChange(self, dates, excludeTypes=None):
        return self.model.calc(
            dates, excludeTypes=excludeTypes
    ...
    def update(self, codes=None, calcSummary=None):
        ...
        if calcSummary:
            result = pd.DataFrame(
                summary, columns=StationCoordModelUpdater.summary_cols
            )
        return result
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Refresh station coordinate models by linear fitting to data"
    )
    parser.add_argument(
        "config_file", nargs="?", help="Station prediction model configuration file"
    )
    parser.add_argument("-c", "--codes", nargs="+", help="Station codes to evaluate")
    parser.add_argument(
        "-r", "--auto-reject", action="store_true", help="Auto reject observations"
    )
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="Generate more output"
    )
    parser.add_argument(
        "-s", "--summary-file", help="Name of change/residual summary CSV file"
    )

    args = parser.parse_args()
    options = dict(auto_reject_obs=args.auto_reject)
    ...
    if summary is not None and args.summary_file:
        summary.to_csv(
            args.summary_file, index=False, float_format="%.4f", date_format="%Y-%m-%d"
        )
