...
from .GDB_Timeseries import GDB_Timeseries_Calculator

from LINZ.Geodetic.Ellipsoid import GRS80
from LINZ.Geodetic import GDB

help_file='cors_gdb_editor.html'
...
class AppForm(QMainWindow):
    ...
    def __init__(self, cfgfile=None, options=None, parent=None):
        ...
        self.gdbcalculator=None
        self.deformationModelVersion=None
        self.timeseriesList=None
        self.solutiontypes=None
        ...
        self.autoscale=False
        self.plots=None
        self.tsCalculator=None
        self.obsplots=None
        self.setWindowTitle('GDB - CORS comparison')
        self.createActions()
        self.createMenu()
    ...
    def read_config(self,cfgfile,options=None):
        required='deformation_model_directory timeseries_file update_csv_file'.split()
        config={x:None for x in 'deformation_model_directory timeseries_file update_csv_file solutiontypes'.split()}
        ...
        for cfg in required:
            if cfg not in config:
                raise RuntimeError('Configuration must define '+cfg)

        self.config=config
        self.gdbcalculator=GDB_Timeseries_Calculator(config.get('deformation_model_directory'))
        self.deformationModelVersion=self.gdbcalculator.deformationModelVersion()
        self.timeseriesList=TimeseriesList(config.get('timeseries_file'))
        self.updateCsvFile=config.get('update_csv_file')

        solutiontypes=config.get(solutiontypes,'')
    ...
    def on_about(self):
        msg = """ 
        GDB - CORS comparison tool

        View CORS time series compare with GDB coordinates
        """
    ...
    def saveModel( self ):
        if not self.model:
            return
        self.saveUpdates()
        self.statusText.setText('Updates saved')
    ...
    def loadModel(self,code)
        timeseries=self.timeseriesList.get(code,self.solutiontypes)
        gdbtimeseries=self.
