    ...
    def getConfigItem(item, default=None, lookup=None):
        ...
        try:
            ...
            if value == "":
                ...
                value = re.sub(
                    r"\{(\w+)\}",
                    lambda m: str(lookup.get(m.group(1), m.group(0))),
                    value,
                )
                value = re.sub(
                    r"\{(\w+)\}",
                ...
                value = re.sub(
                    r"\{(\w+)\}",
                    lambda m: cfg.get(config_section, m.group(1), m.group(0)),
                    value,
                )
        except:
            value = default
...
def filterNewWarnings(warnings, notifiedFile, reminderDays, expiryDays):
    ...
    for w in warnings:
        try:
            ...
            found = [x for x in notified if (x["code"], x["warning_type"]) == match]
...
def main():
    ...
    if len(warnings) > 0:
        if notificationFile is not None:
            reminderDays = int(
                config("notification_reminder_frequency_days", default="30")
            )
            expiryDays = int(config("notification_expiry_days", default="30"))
            warnings, notified = filterNewWarnings(
                warnings, notificationFile, reminderDays, expiryDays
            )

    warning_message = config("no_warning_message")
    if len(warnings) > 0:
        warning_message = "\n".join(
            [config("warning_message", lookup=w) for w in warnings]
        )
